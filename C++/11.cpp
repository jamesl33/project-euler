...
int GridProduct(int arr[20][20], int x, int y, int dirX, int dirY)
{
    int sum = 1;

    for (int i = 0; i < 4; i++)
    {
        sum *= arr[y + (i * dirY)][x + (i * dirX)];
    }
...
int main() {
    ...
    for (int y = 0; y < 20; y++)
    {
        for (int x = 0; x < 20; x++)
        {
            if (x + 4 <= 20)
            {
                max_product = std::max(max_product, GridProduct(numbers, x, y, 1, 0));
            }
            ...
            if (y + 4 <= 20)
            {
                max_product = std::max(max_product, GridProduct(numbers, x, y, 0, 1));
            }
            ...
            if (x + 4 <= 20 && y + 4 <= 20)
            {
                max_product = std::max(max_product, GridProduct(numbers, x, y, 1, 1));
            }
